...
1. **🔤 中英双语第一优先**：**任何前端界面更改，必须同时适配中文和英文两个语言界面！** 在编写任何新UI元素时，第一时间就把中英文翻译都加入 `i18n` 对象，确保不会遗漏。这一条和Git提交同等重要，每次提交前必须检查双语支持是否完善！
2. **Git 提交与 GitHub Pages 同步**：每次功能更改后都要提交 git 并推送到 main 分支，GitHub Pages 会自动部署
3. **模块分类整理**：策略工坊、因子库、战术优化等模块需要做好分类整理，便于后续自定义扩充
4. **交互功能完整性**：所有按钮和交互模块必须有实际功能，不能只是装饰
5. **全局配置优先**：市场切换、语言切换等功能应设置为全局按钮
